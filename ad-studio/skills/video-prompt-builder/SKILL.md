...
description: Generate one Seedance 2.0 Reference-to-Video prompt from an approved visual storyboard board image. Use this skill after ad-storyboard-builder has produced the single storyboard image. The final prompt must reference that storyboard image through reference_image_url and describe the 5-cut, 15-second video. Do not generate multiple prompt variants or separate per-cut prompts.
---

# Video Prompt Builder for Seedance 2.0

Build one cinematic Seedance 2.0 Reference-to-Video prompt from the approved storyboard board image. The storyboard image is the primary visual reference and must be passed to Seedance as `reference_image_url`.

## Single-video contract

This skill produces exactly **one** final video prompt from the approved storyboard image.

- One storyboard image enters this skill.
- One Seedance 2.0 Reference-to-Video prompt exits this skill.
- The output must describe one 15-second, 5-cut, video in the selected aspect ratio.
- The prompt must include `reference_image_url = [storyboard image URL/path]` or a clearly named placeholder if the image has not been uploaded yet.
- Do not generate alternate video concepts, multiple prompt versions, or separate per-cut videos inside the same run. If the user explicitly asks for variants, each variant must be treated as a separate run with its own storyboard image and final prompt.

## How this skill works

1. The user provides a **visual storyboard board image** generated by `ad-storyboard-builder`, plus any compact notes from the brief.
...
3. Generate one complete Seedance prompt in plain text, structured into the four mandatory sections below, and include the storyboard image as `reference_image_url`.

## Input expectations

The user's brief can include any combination of:
- Storyboard image path or URL
...
If the storyboard image path/URL is missing, ask for it or use a clear placeholder. Do not proceed as if separate character/product/cut references exist by default.
...
reference_image_url: [storyboard image URL/path]
duration: 15s
aspect_ratio: 16:9
cut_count: 5
```
